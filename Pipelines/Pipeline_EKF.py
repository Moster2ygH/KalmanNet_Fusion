        ...
        for ti in range(0, self.N_steps):
            ...
            if(randomInit):
                ...
                for index in n_e:
                    ...
                    ii += 1
                self.model.InitSequence(train_init_batch, SysModel.T) #这里model是神经网络模型
            else:
            ...
            for t in range(0, SysModel.T):
                x_out_training_batch[:, :, t] = torch.squeeze(self.model(torch.unsqueeze(y_training_batch[:, :, t],2))) #这里直接就是forward方法
            ...
            with torch.no_grad():
                ...
                if (self.MSE_cv_dB_epoch[ti] < self.MSE_cv_dB_opt):
                    ...
                    epochs_no_improve = 0  # Reset counter if we get a new best validation loss
                else:
                    epochs_no_improve += 1

                # Early stopping check
                if epochs_no_improve >= early_stopping_patience:
                    print(f'Early stopping at epoch {ti}')
                    break
            ########################
            ### Training Summary ###
            ########################
