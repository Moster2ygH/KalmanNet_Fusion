...
print("m1x_0=", m1x_0)
m2x_0 = std_feed * std_feed * torch.eye(
    m)  # Initial Covariance for feeding to filters and KNet 初始协方差，std_feed=0表示训练时候知道初始位置
...
#########################################
sys_model_gen = SystemModel(f, Q, hRotate, R, args.T, args.T_test, m, n)  # parameters for GT
...
[train_input_long, train_target_long, cv_input, cv_target, test_input, test_target, _, _, _] = torch.load(
...
sys_model_partial.InitSequence(m1x_00, m2x_0)
# Model for 2nd pass
...
[MSE_EKF_linear_arr, MSE_EKF_linear_avg, MSE_EKF_dB_avg, EKF_KG_array, EKF_out] = EKFTest(args, sys_model, test_input,
                                                                                          test_target)
...
[MSE_EKF_linear_arr_partial, MSE_EKF_linear_avg_partial, MSE_EKF_dB_avg_partial, EKF_KG_array_partial,
 EKF_out_partial] = EKFTest(args, sys_model_partial, test_input, test_target)
...
####################################################################################
    ...
    # else:
    #     [MSE_cv_linear_epoch, MSE_cv_dB_epoch, MSE_train_linear_epoch, MSE_train_dB_epoch] = KNet_Pipeline.NNTrain(
    #         sys_model_partial, cv_input, cv_target, train_input, train_target, path_results)
    ## Test Neural Network
    [MSE_test_linear_arr, MSE_test_linear_avg, MSE_test_dB_avg, KNet_out, RunTime] = KNet_Pipeline.NNTest(
        sys_model_partial, test_input, test_target, path_results)
...
file_path2 = os.path.join(new_folder_path, 'KF_out_partial.pt')
...
torch.save(EKF_out_partial, file_path2)
...
import csv
file_path_save = os.path.join(base_folder_path, 'test_results.csv')
# 检查文件是否存在，如果不存在则创建并写入标题行
file_exists = os.path.isfile(file_path_save)

with open(file_path_save, mode='a', newline='') as file:
    writer = csv.writer(file)
    if not file_exists:
        # 如果文件不存在，写入标题行
        writer.writerow(['m1x_0', 'm1x_00', 'std_gen', 'std_feed', 'MSE_EKF_dB_avg',
                         'MSE_EKF_dB_avg_partial', 'MSE_test_dB_avg'])

    # 写入当前训练的结果
    writer.writerow([m1x_0, m1x_00, std_gen, std_feed, MSE_EKF_dB_avg,
                     MSE_EKF_dB_avg_partial, MSE_test_dB_avg])

print(f"训练结果已保存到 {file_path_save}")
