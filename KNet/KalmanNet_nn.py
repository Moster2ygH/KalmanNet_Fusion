...
class KalmanNetNN(torch.nn.Module):
    ...
    #         x_out_test[:,:, t] = torch.squeeze(self.model(torch.unsqueeze(test_input[:,:, t],2))) test_input：200（样本数） X 3 X T ，test_input[:,:, t]就是200X3 所以要增加一维保持一致，运行结束再减去一维
    def forward(self, y): #输入y是量测 输出 return self.m1x_posterior
        y = y.to(self.device)
