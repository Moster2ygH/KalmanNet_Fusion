...
# 0 获取当前的日期和时间
base_folder_path = 'Results/saved_lor_Discrete-Time/test/'
...
# path_results = 'KNet/'
path_results = load_model_path + '20240711_1559'  # new_folder_path = os.path.join(base_folder_path, formatted_datetime)
...
dataFileName = ['data_lor_v20_RVdb0.1-20_3060_test.pt']
...
m2x_0 = torch.diag(torch.tensor([25.0, 4.0, 9.0]))

args.x0_distributions = [
...
# 定义测试时候采用的分布
#修改x0_distributions_test需要在
x0_distributions_test = [
    {'mean': torch.tensor([[1.0], [1], [1.0]]),
     'std_dev': torch.diag(torch.tensor([1.0, 1.0, 1.0]))},
    {'mean': torch.tensor([[-20.0], [2], [1.0]]),
     'std_dev': torch.diag(torch.tensor([2.0, 2.0, 2.0]))},
    {'mean': torch.tensor([[10.0], [5], [2.0]]),
     'std_dev': torch.diag(torch.tensor([5.0, 2.0, 3.0]))},
    {'mean': torch.tensor([[60.0], [10], [3.0]]),
     'std_dev': torch.diag(torch.tensor([20.0, 8.0, 9.0]))},
    {'mean': torch.tensor([[-10.0], [15], [5.0]]),
     'std_dev': torch.diag(torch.tensor([30.0, 10.0, 4.0]))},
]
...
print(f"Similarity between training and test distributions: {dist_similarity}")
# total_distance = 0.0
#
# # 计算并累加所有分布对之间的距离
# for i in range(len(x0_distributions_train)):
#     for j in range(len(x0_distributions_test)):
#         dist = mahalanobis_distance(
#             x0_distributions_train[i]['mean'],
#             x0_distributions_train[i]['std_dev'],
#             x0_distributions_test[j]['mean'],
#             x0_distributions_test[j]['std_dev']
#         )
#         total_distance += dist
# distributions_distance = total_distance
# print(f"Total distance between training and test distributions: {distributions_distance}")
# 定义参数 参数写进txt文件
...
sys_model = SystemModel(f, Q, hRotate, R, args.T_short, args.T_test, m, n, x0_distributions_test)  # parameters for GT
sys_model.InitSequence(m1x_0, m2x_0)  # x0 and P0

# Model with partial info
sys_model_partial = SystemModel(f, Q, h, R, args.T_short, args.T_test, m, n, x0_distributions_test)
sys_model_partial.InitSequence(m1x_0, m2x_0)
# Model for 2nd pass
sys_model_pass2 = SystemModel(f, Q, h, R, args.T_short, args.T_test, m, n, x0_distributions_test)  # parameters for GT
...
[MSE_EKF_linear_arr, MSE_EKF_linear_avg, MSE_EKF_dB_avg, EKF_KG_array, EKF_out] = EKFTest(args, sys_model, test_input, test_target, randomInit = True, test_init=test_init)
...
[MSE_EKF_linear_arr_partial, MSE_EKF_linear_avg_partial, MSE_EKF_dB_avg_partial, EKF_KG_array_partial,
 EKF_out_partial] = EKFTest(args, sys_model_partial, test_input, test_target, randomInit = True, test_init=test_init)
...
if switch == 'full':
    ...
    [MSE_test_linear_arr, MSE_test_linear_avg, MSE_test_dB_avg, KNet_out, RunTime] = KNet_Pipeline.NNTest(sys_model,
                                                                                                          test_input,
                                                                                                          test_target,
                                                                                                          path_results, randomInit=True, test_init=test_init)
...
torch.save({
    ...
    'KNet_out': KNet_out,
    'MSE_test_linear_arr': MSE_test_linear_arr,
...
result_file_path_save = os.path.join(base_folder_path, 'test_results_distri.csv')
...
with open(result_file_path_save, mode='a', newline='') as file:
    ...
    if not file_exists:
        # 如果文件不存在，写入标题行
        writer.writerow(
            ['formatted_datetime', 'chop', 'args.T', 'args.T_short', 'args.randomInit_train', 'args.randomInit_test',
             'KnownRandInit_train ',
             'r2', 'm1x_0', 'm2x_0', 'm2x_0_gen', 'std_gen', 'std_feed', 'dist_similarity',
             'MSE_EKF_dB_avg', 'MSE_EKF_dB_avg_partial', 'MSE_test_dB_avg'])

    # 写入当前训练的结果
    writer.writerow([formatted_datetime, chop, args.T, args.T_short, args.randomInit_train, args.randomInit_test,
                     KnownRandInit_train,
                     r2, m1x_0, m2x_0, m2x_0_gen, std_gen, std_feed, dist_similarity,
                     MSE_EKF_dB_avg, MSE_EKF_dB_avg_partial, MSE_test_dB_avg])
...
args_result_txt_file_path = os.path.join(new_folder_path, 'test_results_distri.txt')
with open(args_result_txt_file_path, 'w') as file:
    for key, value in result_data_dict.items():
        file.write(f"{key} = {value}\n")
# 输出文件路径，方便确认
print(f"测试参数已保存到 {args_result_txt_file_path}")
####################
####################
...
Plot.plotTraj_CA(test_target, EKF_out, KNet_out, dim=0, file_name=new_folder_path + PlotfileName0)  # Position
Plot.plotTraj_CA(test_target, EKF_out, KNet_out, dim=1, file_name=new_folder_path + PlotfileName1)  # Velocity
Plot.plotTraj_CA(test_target, EKF_out, KNet_out, dim=2, file_name=new_folder_path + PlotfileName2)  # Acceleration
