...
class Plot_RTS(Plot_KF):
    ...
    def plotTraj_CA(self,test_target, RTS_out, rtsnet_out, dim, file_name):
        legend = ["KFNet", "Ground Truth", "EKF_out_partial"]
        font_size = 14
        ...
        if dim==0:#position
            ...
            plt.plot(x_plt, RTS_out[0][0,:].cpu().detach().numpy(), label=legend[2])
            ...
            plt.plot(x_plt, RTS_out[0][1,:].cpu().detach().numpy(), label=legend[2])
            plt.legend(fontsize=font_size)
            plt.xlabel('t', fontsize=font_size)
            ...
            plt.plot(x_plt, RTS_out[0][2,:].cpu().detach().numpy(), label=legend[2])
            plt.legend(fontsize=font_size)
            plt.xlabel('t', fontsize=font_size)
