...
def general_settings():
    ...
    parser.add_argument('--distribution', type=str, default='normal', metavar='distribution',
                        help='input distribution for the random initial state (uniform/normal)') #(均匀/正态分布)

    # 添加 x0_distributions 参数
    parser.add_argument('--x0_distributions', type=list,
                        default='[{"mean": [1.0, 1.0, 1.0], "std_dev": [1.0, 1.0, 1.0]}]',
                        help='Initial state distributions in JSON format. Example: \'[{"mean": [1.0, 1.0, 1.0], "std_dev": [1.0, 1.0, 1.0]}]\'')
