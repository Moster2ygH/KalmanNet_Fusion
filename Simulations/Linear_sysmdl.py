...
1 Store system model parameters:  存储系统模型参数
    state transition matrix F,  状态转换矩阵 F
    observation matrix H, 观测矩阵 H
    process noise covariance matrix Q, 过程噪声协方差矩阵 Q
    observation noise covariance matrix R, 观测噪声协方差矩阵 R
    train&CV dataset sequence length T, 数据集序列长度 T
    test dataset sequence length T_test, etc.  测试数据集序列长度 T_test 等

2 Generate dataset for linear cases 生成线性案例数据集
