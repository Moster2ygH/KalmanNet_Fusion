...
class SystemModel:
    ...
    def __init__(self, f, Q, h, R, T, T_test, m, n, x0_distributions_gen, prior_Q=None, prior_Sigma=None, prior_S=None):
        ...
        self.T = T
        self.T_test = T_test
        self.x0_distributions_gen = x0_distributions_gen
        #########################
        ### Covariance Priors ###
        #########################
        if prior_Q is None:
    ...
    def GenerateBatch(self, args, size, T, randomInit=False): #size = args.N_E 总样本数即总轨迹数
        ...
        print("选择哪一个分布进行初始状态生成，args.distribution = ", args.distribution)
        if(randomInit):
            ...
            if args.distribution == 'uniform':
                ...
                for i in range(size):
                    # 随机选择一个分布
                    x0_distributions = self.x0_distributions_gen
